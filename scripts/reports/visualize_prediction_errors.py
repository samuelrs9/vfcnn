#!/usr/bin/env python3
"""
Script to visualize prediction errors (false positives and false negatives).
Uses only the "sparse no coarse" prediction from compare_predictions.yaml configuration.
Generates a 3D image for each simulation using the last step.
"""
...
def visualize_errors(particles, gt_labels, pred_labels, sim_name, output_path, step):
    ...
    # Subplot 2: Apenas Falsos Positivos
    if n_fp > 0:
        fig.add_trace(
            go.Scatter3d(
                x=x[false_positives], y=y[false_positives], z=z[false_positives],
    ...
    # Subplot 3: Apenas Falsos Negativos
    if n_fn > 0:
        fig.add_trace(
            go.Scatter3d(
                x=x[false_negatives], y=y[false_negatives], z=z[false_negatives],
    ...
    # Update layout
    fig.update_layout(
        title_text=f'Error Analysis - {sim_name} - Step {step}',
        width=1800,
        height=600,
        showlegend=True,
...
def main():
    ...
    output_dir = '/work1/voxel-fluid-net/reports/prediction_errors'
