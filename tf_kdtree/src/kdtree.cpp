							...
							const point_i_knn_t nr_nns_searches, const uint8_t metric)
{
    ...
    compDistLeafPartition<T, T_calc, dims>(point, partition_leaf, best_dists.data(), best_idx.data(), nr_nns_searches, metric);
}
                        ...
                        const point_i_knn_t nr_nns_searches, const uint8_t metric)
{
    ...
    const tree_visit_f<T> func = [&tree, &point, &point_proj, &worst_dist, &metric](const Partition<T>& current_partition, 
        const NodeTag tag, const tree_ind_t current_lin_ind, const level_t current_level)
    {
        ...
        else
        {
            ...
            else //Node has been used in the correct side, now we use the non-matching site: Project and test if descent is necessary
            {
                ...
                if(partitionNecessary<T, T_calc, dims>(point, point_proj, current_partition, worst_dist, metric))
    ...
    do
    {
        ...
        compDistLeafPartition<T, T_calc, dims>(point, partition_leaf, best_dists, best_idx, nr_nns_searches, metric);
...
template
<typename T, typename T_calc, dim_t dims>
void compDistNode(const std::array<T, dims>& point, const Partition<T>* partitions, const PartitionLeaf<T, dims>* leaves,
                        ...
                        const point_i_knn_t nr_nns_searches, const uint8_t metric)
{
    ...
    if(current_level == 0)
    {
        ...
        compDistLeafPartition<T, T_calc, dims>(point, partition_leaf_first, best_dists, best_idx, nr_nns_searches, metric);
        if(partitionNecessary<T, T_calc, dims>(point, point_proj, partition, best_dists.back(),metric))
        {
            const PartitionLeaf<T, dims>& partition_leaf_second = (!lower_than_median ? 
                                                leaves[TreeTraversal<T, dims>::compLeftLeafInd(lin_ind)] : 
                                                leaves[TreeTraversal<T, dims>::compRightLeafInd(lin_ind)]);
            compDistLeafPartition<T, T_calc, dims>(point, partition_leaf_second, best_dists, best_idx, nr_nns_searches, metric);
        }
    }
    else
    {
        ...
        compDistNode<T, T_calc, dims>(point, partitions, leaves, lower_than_median ? left_child_ind : right_child_ind, 
					best_dists, best_idx, current_level-1, point_proj, nr_nns_searches, metric);
        ...
        if(partitionNecessary<T, T_calc, dims>(point, point_proj, partition, best_dists.back(), metric))
        {
            ...
            compDistNode<T, T_calc, dims>(point, partitions, leaves, !lower_than_median ? left_child_ind : right_child_ind,
                best_dists, best_idx, current_level-1,point_proj_new, nr_nns_searches, metric);
        }
    }    
}
...
template
<typename T, typename T_calc, dim_t dims>
void compDistNode(const std::array<T, dims>& point, const std::vector<Partition<T>>& partitions, 
    const std::vector<PartitionLeaf<T, dims>>& leaves,const tree_ind_t lin_ind,std::vector<T>& best_dists, 
    std::vector<point_i_knn_t>& best_idx,const int current_level, const std::array<T, dims>& point_proj, 
    const point_i_knn_t nr_nns_searches, const uint8_t metric)
{
    compDistNode<T, T_calc, dims>(point, partitions, leaves, lin_ind, best_dists, best_idx, 
        current_level, point_proj, nr_nns_searches, metric);
}
...
void KDTreeKNNSearch(PartitionInfo<T, dims>& partition_info,
                    ...
                    point_i_knn_t* idx, const point_i_knn_t nr_nns_searches, const uint8_t metric)
{
	...
	for (point_i_t query_i = 0; query_i < nr_query; query_i++)
    {
        ...
        searchKDTreeIteratively<T, T_calc, dims>(query_point, partition_info, 
            best_dists, best_i, nr_nns_searches, metric);
        ...
        compDistNode<T, T_calc, dims>(query_point, partitions, leaves,  0, best_dists2, 
            best_i2, levels-1, query_point, nr_nns_searches, metric);
...
template void KDTreeKNNSearch<float, float, 3>(PartitionInfo<float, 3>& partition_info,
    const point_i_t nr_query, const std::array<float, 3>* points_query, float * dist, point_i_t* idx, 
    const point_i_knn_t nr_nns_searches, const uint8_t metric);
