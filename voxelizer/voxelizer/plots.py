...
class Plots2D:
    ...
    def __init__(self,data_reader=None,ndim=None,num_figs=None,voxelizer=None):
        """ 
        Construtor. 
        ...
        if data_reader!=None:
            ...
            self.limits = self.limits.reshape(-1,self.ndim)            
        else:
            if ndim is None:
                ndim = 2
            self.ndim = ndim
            if self.ndim==2:
                ...
                self.limits = np.array([[0,1],[0,1],[0,1]])
    ...
    def draw_images_dataset(self,dataset,key_dense=None,keys_sparse=None):
        """
    ...
    def draw_image(self,image,position=[0,0],length=[1.0,1.0],
    ...
    def draw_voxels(self,voxels_idx,linecolor='r',draw_index=False):
        """ 
        ...
        """
        voxels_ij = self.voxelizer.flat_to_multi_index(voxels_idx)
        ci,cj = voxels_ij[1,:],voxels_ij[0,:]
        left_bottom_coords = np.array(
            [self.voxelizer.global_coord[0][cj],
            self.voxelizer.global_coord[1][ci]]).T
        ...
        if draw_index:
            self.draw_text(left_bottom_coords[:,0],
                           ...
                           [str(x) for x in voxels_idx],
                           fontsize=10)
            
        self.ax.axis('equal')

    def draw_cell_cover_bkp(self,cell_cover_flat,pgrid,grid_id,linecolor='b',draw_index=False):
        """ 
        Desenha uma cobertura de células.
        Útima modificação: 04/08/2021.
